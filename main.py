...
class Main:
    ...
    def train_iter(self):
        ...
        for i, data in enumerate(train_data):
            loss, _ = self.model(**data,Train=True) #调用BertWordPair类中的forward函数，返回(loss),(tag)
    ...
    def evaluate_iter(self, dataLoader=None):
        ...
        for i, data in enumerate(dataiter):
            with torch.no_grad():
                _, (pred_ent_matrix, pred_rel_matrix, pred_pol_matrix) = self.model(**data,Train=False)
    ...
    def test(self):
        ...
        # PATH = os.path.join(self.config.target_dir, "zh_26.pth.tar")
    ...
    def forward(self):
        ...
        self.model = BertWordPair(self.config).to(config.device)# #调用构造函数__init__()， 后面model(**data)时才调用forward
